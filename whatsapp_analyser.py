...
from processor import wht_get_stats , busy_users, get_stats_graph,most_used_words,most_emoji_used
...
def wht_app(df, user):
    ...
    cl1.header('Chat Analysis From')
    cl1.write(f"<h2 style = 'color: green'> {df['time'].iloc[0]} </h2>", 
                unsafe_allow_html=True)
    
    cl2.header('Chat Analysis Till')
    cl2.write(f"<h2 style = 'color: red'> {df['time'].iloc[-1]} </h2>", 
                unsafe_allow_html=True)
    ...
    table_html, table= wht_get_stats(df,user)
